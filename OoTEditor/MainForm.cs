...
public class MainForm : Form
{
    ...
    private void BuildUI()
    {
        ...
        var menuExportHeader = new ToolStripMenuItem("Export as C header...");
        menuExportHeader.Click += OnExportAsHeader;
        ...
        fileMenu.DropDownItems.Add(menuExportHeader);
        fileMenu.DropDownItems.Add(new ToolStripSeparator());
    ...
    private void OnExportAsHeader(object? sender, EventArgs e)
    {
        if (_entries.Count == 0)
        {
            MessageBox.Show("No messages loaded.", "Nothing to export",
                MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        ...
        using var dlg = new SaveFileDialog
        {
            ...
            FileName = "message_data_static_NES.h",
        };
        if (dlg.ShowDialog() != DialogResult.OK) return;

        try
        {
            string content = SohExporter.Export(_entries);
            File.WriteAllText(dlg.FileName, content, new System.Text.UTF8Encoding(encoderShouldEmitUTF8Identifier: false));
            SetStatus($"Exported C header to {Path.GetFileName(dlg.FileName)}.");
        }
        catch (Exception ex)
        {
